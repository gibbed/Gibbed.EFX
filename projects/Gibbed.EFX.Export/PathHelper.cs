...
namespace Gibbed.EFX.Export
{
    internal static class PathHelper
    {
        public static string GetRelativePath(string fromPath, string toPath)
        {
            ...
            {
                var fromPathRoot = Path.GetPathRoot(fromPath);
                var toPathRoot = Path.GetPathRoot(toPath);
                if (string.Compare(fromPathRoot, toPathRoot, StringComparison.OrdinalIgnoreCase) != 0)
                {
                    return null;
                }
            }
